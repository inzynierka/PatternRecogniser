namespace PatternRecogniser.Messages.ExperimentList
{
    public class ExperimentListStringMesseges
    {
        public string listAlreadyExist => "Lista już istnieje";
        public string listOrExperimentDontExist => "Lista lub eksperyment nie istnieje";
        public string listDontExisit => "Lista nie istnieje";
        public string notFoundExperimentsToAdd => "Nie masz żadnych eksperymentów możliwych do dodania";
        public string experimentHasBeenAdded => "Pomyślnie dodano eksperyment";
    }
}
