...
namespace PatternRecogniser.Messages.Validators
{
    public class AuthentycationValidatorLogIn : AbstractValidator<LogIn>
    {
        ...
        public AuthentycationValidatorLogIn(IGenericRepository<User> AuthenticationRepo, IPasswordHasher<User> passwordHasher)
        {

            
            User user = null;
            When(x => {
                user = AuthenticationRepo.Get(u => u.login == x.login).FirstOrDefault();
                return user == null;
            }, ()=>
            {
